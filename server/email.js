...
// The invite code is a single reusable string (see invite_codes table /
// server/routes/auth.js) — anyone who signs up through this link is
// granted VIP access immediately, no password prompt needed.
export async function sendWaitlistVipInvite(to, inviteCode = "COSMEOBETA") {
  const signupUrl = `${APP_URL}/invite/${inviteCode}`;
  await sendEmail({
    to,
    subject: "Your exclusive invite to cosmeo is here! 🌟",
    text: `You're in! 🌟\n\nYou've been hand-selected as one of our first featured creators on cosmeo — Georgia's cosplay marketplace.\n\nJust tap your invite link to create your account — you'll get full access right away, no waiting:\n\n${signupUrl}\n\nWe can't wait to see your work on the platform.\n\n— The cosmeo team`,
    html: `
      <div style="font-family:sans-serif;max-width:480px;margin:0 auto;background:#faf9ff;border-radius:24px;padding:32px;">
        <h1 style="color:#8b72c8;font-size:28px;margin-bottom:4px;">✨ cosmeo</h1>
        <p style="color:#6b7280;margin-top:0;">Georgia's cosplay marketplace</p>
        ...
        <p style="color:#374151;line-height:1.6;">Tap your invite link below to create your account — you'll get full access right away, no waiting:</p>
        <a href="${signupUrl}" style="display:inline-block;margin:20px 0;background:#8b72c8;color:#fff;font-weight:700;font-size:14px;border-radius:12px;padding:12px 24px;text-decoration:none;">${signupUrl}</a>
