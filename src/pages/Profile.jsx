      ...
      <div className="p-4 pt-6">
        ...
        <div className="bg-white rounded-[2.5rem] p-6 text-center flex flex-col items-center relative overflow-hidden card-shadow">
          ...
          <Avatar className="h-28 w-28 border-4 border-white mb-4 z-10 shadow-lg mt-4">
            <AvatarFallback className="bg-primary text-white text-3xl font-black">MC</AvatarFallback>
          </Avatar>
          
          <h2 className="text-2xl font-black z-10 text-foreground">{user.username}</h2>
          <p className="text-primary text-sm font-bold mb-4 z-10">{user.handle}</p>
          
          <p className="text-base font-medium mb-8 z-10 max-w-[280px] text-muted-foreground">{user.bio}</p>
          
          <div className="flex w-full justify-between bg-muted/50 rounded-3xl p-5 z-10 border border-border/50">
            <div className="flex flex-col items-center flex-1">
              <span className="text-2xl font-black text-foreground">{user.rating}</span>
            ...
            <div className="flex flex-col items-center flex-1">
              <span className="text-2xl font-black text-foreground">{user.sales}</span>
              <span className="text-xs font-bold text-muted-foreground uppercase tracking-wide mt-1">Sales</span>
            </div>
            <Separator orientation="vertical" className="h-12 w-px bg-border/80" />
            <div className="flex flex-col items-center flex-1">
              <span className="text-2xl font-black text-secondary">${user.balance}</span>
        ...
        <Button variant="outline" className="w-full mt-8 h-16 rounded-2xl border-none bg-red-50 text-red-500 hover:bg-red-100 hover:text-red-600 font-bold text-lg card-shadow">
