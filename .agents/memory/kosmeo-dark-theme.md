...
- **Why exclude WaitlistLanding.jsx:** it's the pre-launch page and must always render the dark "cosmos" brand look regardless of the user's stored preference — it uses hardcoded hex colors/gradients (not the `background`/`card` tokens), so it's already immune to the `.dark` class toggle by construction. Don't refactor it to use the semantic tokens or it'll start following the toggle.
